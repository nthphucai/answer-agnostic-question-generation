...
from optimum.onnxruntime.trainer_seq2seq import ORTSeq2SeqTrainer
from optimum.onnxruntime.training_args_seq2seq import ORTSeq2SeqTrainingArguments
from questgen.dataset.build_transformer_format_dataset.data_collator import Text2TextDataCollator
from questgen.models.T5CopyGenerator import T5CopyGenerator
from questgen.trainer.modules.mixed_finetune import BestRatioMixedFineTune
from questgen.trainer.trainer_transformers import BaseTrainer
from questgen.utils.file_utils import load_json_file, logger, read_yaml_file
from questgen.utils.model_utils import assert_not_all_frozen, freeze_embeds
...
def start_train(
    ...
    if model_args.model_config is not None:
        hyperparamter = load_json_file(model_args.model_config)
    ...
    if model_args.model_type == "t5-copy-enhance":
        if model_args.model_config is not None:
            config.update(hyperparamter["t5-enhance"])
        ...
        if model_args.model_config is not None:
            config.update(hyperparamter["t5-base"])
    ...
    if any(
        ...
        init_mixfinetune = BestRatioMixedFineTune.from_dataset(
        ...
        ratio = init_mixfinetune.get_best_ratio()
