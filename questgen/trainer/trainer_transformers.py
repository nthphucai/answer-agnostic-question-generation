...
from graph.modules.models.gcnLM import GCNWithEdgeLabels
...
class BaseTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        context_vector = outputs.encoder_last_hidden_state.mean(
            axis=1
        )  # (bz, dim_sent)

        if len(inputs.keys()) > 4:
            node_features, edge_index, edge_relation = (
                inputs["node_features"],
                inputs["edge_index"],
                inputs["edge_relation"],
            )
            bz, num_node, dim_node = node_features.size()
            context_vector = context_vector.view(bz * num_node, -1)

            node_features = node_features.view(bz * num_node, dim_node)
            edge_index = edge_index.view(2, -1)  # 2, total_e = bz * num_node
            edge_relation = edge_relation.view(-1)  # total_e

            if node_features.shape[0] != self.hidden_channels:
                outputs = outputs
            else:
                gcn_output = self.gcn(
                    x=node_features, edge_index=edge_index, edge_attr=edge_relation
                )
                outputs.encoder_last_hidden_state = torch.cat(
                    (context_vector, gcn_output), axis=1
                )
